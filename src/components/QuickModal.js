...
const QuickModal = ({ closeModal, openModal, isOpen, id, product, addToCart }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/shop/data/${id}`);
                                            ...
                                            <div class="">
                                                <div>
                                                    ...
                                                    {/* <div class="flex mt-3">
                                                        {
                                                            img.map((item, ii) => (
                                                                <div class={`relative w-16 h-8 rounded-full overflow-hidden z-10 cursor-pointer ${selectedImage === data?.images[ii] ? 'border border-black' : 'border border-transparent'}`} title="Black" onClick={() => setSelectedImage(data?.images[ii])}>
